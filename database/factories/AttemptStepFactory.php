<?php

namespace Database\Factories;

use Illuminate\Database\Eloquent\Factories\Factory;
use Illuminate\Support\Facades\Hash;
use Illuminate\Support\Str;
use App\Models\Attempt;
use App\Models\User;
use App\Models\ExerciseStep;
/**
 * @extends \Illuminate\Database\Eloquent\Factories\Factory<\App\Models\User>
 */
class AttemptStepFactory extends Factory
{
    /**
     * The current password being used by the factory.
     */
    protected static ?string $password;

    /**
     * Define the model's default state.
     *
     * @return array<string, mixed>
     */
    public function definition(): array
    {
        return [
            'id_attempt' => Attempt::inRandomOrder()->value('id') ?? Attempt::factory(),
            'id_user' => User::inRandomOrder()->value('id') ?? User::factory(),
            'id_exercise_step' => ExerciseStep::inRandomOrder()->value('id') ?? ExerciseStep::factory(),
            'user_answer' => fake()->word(),
            'is_correct' => fake()->numberBetween(0,1),
            'created_at' => fake()->dateTime('now'),
            'updated_at' => fake()->dateTime('now'),
        ];
    }
}
