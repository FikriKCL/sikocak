...
@if($alreadyCompleted)
<div class="mb-6 bg-[#CCFF00] p-5 sm:p-6 rounded-3xl border-4 border-black shadow-[8px_8px_0]">
    <div class="flex items-center gap-3">
        <svg class="w-7 h-7 sm:w-8 sm:h-8 text-black" fill="currentColor" viewBox="0 0 24 24">
            <path d="M9 16.17L4.83 12l-1.42 1.41L9 19 21 7l-1.41-1.41z"/>
        </svg>
        <span class="text-lg sm:text-xl font-black">Anda sudah menyelesaikan level ini!</span>
    </div>
</div>
@endif
...
<div class="bg-white rounded-3xl border-4 border-black p-5 sm:p-8 shadow-[8px_8px_0]">

    <!-- Exercise Info -->
    <div class="flex flex-col sm:flex-row justify-between items-start sm:items-center mb-6 gap-3 sm:gap-0">
        <div class="bg-[#FF9966] inline-block px-4 sm:px-6 py-2 rounded-full border-3 border-black">
            <span class="text-base sm:text-lg font-black">{{ $exercise->skill->name }}</span>
        </div>
        <div class="bg-gray-100 px-4 sm:px-6 py-2 rounded-full border-3 border-black">
            <span class="text-sm font-black">{{ $exercise->exerciseSteps->count() }} Soal</span>
        </div>
    </div>

    <!-- Question -->
    <h2 class="text-2xl sm:text-3xl font-black mb-6 sm:mb-8 text-black">
        {{ $exercise->question_text }}
    </h2>

    <!-- Steps -->
    @foreach($exercise->exerciseSteps as $stepIndex => $step)
    <div class="mb-4 sm:mb-6 p-4 sm:p-6 bg-gray-50 rounded-2xl border-3 border-black">
        <div class="flex flex-col sm:flex-row items-start sm:items-center gap-3 sm:gap-4 mb-4">
            ...
            <p class="text-base sm:text-lg font-semibold text-black flex-1">{{ $step->content }}</p>
        </div>
        ...
        <div class="ml-0 sm:ml-14">
            <input 
                ...
                class="w-full px-4 sm:px-6 py-3 sm:py-4 rounded-2xl border-4 border-black text-base sm:text-lg font-semibold
                       focus:outline-none focus:ring-4 focus:ring-[#CCFF00] bg-white"
                required
            ...
            <div class="mt-2 flex flex-col sm:flex-row items-start gap-2 sm:gap-4">
...
<div class="flex justify-center">
    <button 
        ...
        class="w-full sm:w-auto
               ...
               transition-all"
    >
        <span class="text-lg sm:text-2xl font-black text-black">Periksa Jawaban</span>
    </button>
</div>
