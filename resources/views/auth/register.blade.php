<x-register-layout>
    <form method="POST" action="{{ route('register') }}">
        ...
        <div class="mt-6">
            <x-text-input id="username" class="block w-full" type="text" name="username" :value="old('username')" placeholder="Username" required autocomplete="username" pattern="^[a-z0-9]+$" oninput="this.value = this.value.toLowerCase().replace(/\s+/g, '')" title="Username harus lowercase dan tanpa spasi."/>
        ...
        <div class="mt-3 rounded-full" x-data="{ show: false }">
            <div class="relative">
                ...
                <button type="button" @click="show = !show" class="absolute inset-y-0 right-3 flex items-center">
                    ...
                    <svg x-show="show" xmlns="http://www.w3.org/2000/svg" class="w-5 h-5" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                        <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M13.875 18.825A10.05 10.05 0 0112 19c-4.477 0-8.268-2.944-9.542-7a10.05 10.05 0 012.28-3.956m3.124-2.484A9.956 9.956 0 0112 5c4.478 0 8.268 2.944 9.543 7a9.958 9.958 0 01-4.304 5.235M15 12a3 3 0 00-3-3m0 0a2.99 2.99 0 00-1.219.257M9.88 9.88L4.22 4.22m15.56 15.56L14.12 14.12" />
                    </svg>
                </button>
            </div>
        ...
        <div class="flex items-center justify-center flex-col mt-4">
            ...
            <div class="my-2">
                Sudah memiliki akun? <a class="underline text-sm text-gray-600 hover:text-gray-900" href="{{ route('login') }}">
                {{ __('Klik disini') }}</a>
            </div>
        </div>
    </form>
